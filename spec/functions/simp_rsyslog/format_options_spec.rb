...
describe 'simp_rsyslog::format_options' do
  ...
  context 'with bad parameters' do
    it {
      is_expected.to run.with_params({}).and_raise_error(%r{not find any valid content})
    }

    it {
      is_expected.to run.with_params({ 'foo' => ['bar'] }).and_raise_error(%r{not find any valid content})
    }
  end
end
