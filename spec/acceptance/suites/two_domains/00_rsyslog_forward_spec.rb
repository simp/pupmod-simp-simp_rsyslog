...
describe 'simp_rsyslog' do
  ...
  hosts_with_role(hosts, 'rsyslog_server1').first
  hosts_with_role(hosts, 'rsyslog_server2').first
    ...
    hosts_with_role(hosts, 'rsyslog_client').each do |client|
      context "#{client} logging to the remote syslog server #{server} with default forwarding" do
        ...
        it "collects #{client} messages to host-specific, server logs, as well as client logs" do
          ...
          # send the messages
          default_test_array.each do |options, message, _server_logfile|
          ...
          default_test_array.each do |_options, message, server_logfile|
