  ...
  hosts_with_role(hosts, 'rsyslog_server').each do |server|
    hosts_with_role(hosts, 'rsyslog_client').each do |client|
      context "#{client} logging to the remote syslog server #{server} with default forwarding" do
        ...
        it "configures server #{server} without errors" do
        ...
        it "configures client #{client} without errors" do
        ...
        it "collects #{client} messages to host-specific, server logs, as well as client logs" do
          ...
          default_test_array.each do |_options, message, server_logfile, client_logfile|
            if server_logfile
            ...
            if client_logfile
              ...
              if ['CLIENT_FORWARDED_NEWS_CRIT_LOG', 'CLIENT_FORWARDED_UUCP_CRIT_LOG'].include?(message)
                # logged to /var/log/spooler AND /var/log/messages because of syslog module bug
                expect(result.stdout.strip).to match(%r{/var/log/#{client_logfile}})
              else
